...
contract AutomatedVoting is IAutomatedVoting, GovernorModule {
	...
	function isDuringVoting(uint256 _election) public view override returns (bool) {
		return
			...
			block.timestamp > elections[_election].startTime + 1 weeks && block.timestamp <= electionEndTime(_election);
	}
	...
	function timeUntilNextScheduledElection() public view override returns (uint256) {
		if (block.timestamp >= lastScheduledElectionStartTime + SIX_MONTHS) {
			...
			return SIX_MONTHS - (block.timestamp - lastScheduledElectionStartTime);
		}
	}
	...
	function timeUntilElectionStateEnd(uint256 _election) public view override returns (uint256) {
		...
		if (electionEndTime(_election) <= block.timestamp || _election >= electionCounter) {
			return 0;
		} else {
	...
	/// @inheritdoc IAutomatedVoting
	function getCandidateAddress(uint256 _election, uint256 index) public view override returns (address candidate) {
	...
	function isElectionFinalized(uint256 _election) public view override returns (bool) {
		if (
			elections[_election].status == Enums.status.finalized || elections[_election].status == Enums.status.invalid
		) {
			return true;
		} else {
			return false;
	...
	function startScheduledElection() public override {
		if (
			...
			(lastScheduledElectionNumber == 0 && block.timestamp < epochStartTime)
		) {
	...
	function startCouncilElection(address _memberToRemove) public override onlySafe notDuringScheduledElection {
	...
	function startCommunityElection() public override notDuringScheduledElection {
	...
	function nominateCandidate(
		uint256 _election,
		address candidate
	) public override wasStakedBeforeElection(_election) onlyDuringNomination(_election) {
	...
	function _nominate(uint256 election, address candidate) internal {
		...
		if (elections[election].theElectionType == Enums.electionType.replacement && isCouncilMember(candidate)) {
	...
	function vote(
		uint256 _election,
		address candidate
	) public override wasStakedBeforeElection(_election) onlyDuringVoting(_election) {
		...
		uint newNumberOfVotes = elections[_election].voteCounts[candidate] +
			stakingRewardsV2.balanceAtTime(msg.sender, elections[_election].startTime); //todo: test for staking amounts addition
		elections[_election].candidateAddresses = _sortCandidates(_election, candidate, newNumberOfVotes);
	...
	function isCouncilMember(address voter) public view returns (bool isACouncilMember) {
		for (uint i = 0; i < council.length; i++) {
			if (council[i] == voter) {
				return true;
			}
	...
	function _finalizeElection(uint256 _election) internal {
		...
		if (elections[_election].theElectionType == Enums.electionType.scheduled) {
			...
			//todo: commented out because tests need to be fixed (AutomatedVoting.t.sol needs safe setup)
		} else if (elections[_election].theElectionType == Enums.electionType.community) {
		...
		} else if (elections[_election].theElectionType == Enums.electionType.replacement) {
	...
	function _sortCandidates(
		...
		for (uint256 i = 0; i < candidates.length; i++) {
			...
			else if (elections[_election].voteCounts[candidate] < newNumOfVotes) {
